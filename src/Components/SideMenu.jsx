      ...
      <div>
        ...
        <ul className="flex flex-col space-y-2">
          {links.map((link, index) => {
            return (
              <li className="flex p-3 cursor-pointer rounded-lg flex-row-reverse justify-center lg:justify-start items-center gap-2 group bg-white hover:bg-gradient-to-l from-[#4D7DFF] to-[#1C48C2]" key={index}>
                <i className={`${link.logo} text-[#737791] group-hover:text-white duration-300`}></i>
                <p className="text-[#737791] text-right group-hover:text-white duration-300 hidden lg:block">{link.title}</p>
              </li>
            );
          })}
        </ul>
